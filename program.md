...
4. **Verify data exists**: Check that `~/.cache/autoresearch/mnist/` contains the MNIST files. If not, tell the human to run `uv run prepare.py`.
...
**What you CANNOT do:**
- Modify `prepare.py`. It is read-only. It contains the fixed data prep, training constants, and evaluation metric.
- Install new packages or add dependencies. You can only use what's already in `pyproject.toml`.
- Modify the evaluation harness. The `evaluate_accuracy` function in `prepare.py` is the ground truth metric.

**The goal is simple: get the highest val_acc.** Since the time budget is fixed, you don't need to worry about training time — it's always 5 minutes. Everything is fair game: change the architecture, the optimizer, the hyperparameters, the batch size, the model size. The only constraint is that the code runs without crashing and finishes within the time budget.
...
2. val_acc achieved (e.g. 0.923400) — use 0.000000 for crashes
...
```
commit	val_acc	memory_gb	status	description
a1b2c3d	0.923400	0.1	keep	baseline
b2c3d4e	0.926100	0.1	keep	increase LR to 0.01
c3d4e5f	0.921800	0.1	discard	switch to SGD
d4e5f6g	0.000000	0.0	crash	broken input normalization
```
